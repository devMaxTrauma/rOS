                    ...
                    if rKernel.get_key("ros_distance_calculating_enabled").value:
                        ...
                        if rKernel.get_key("ros_distance_display_enabled").value and class_average_width != -1.0:
                            if rKernel.get_key("ros_distance_unit").value == "meter":
                                distance = class_average_width / (
                                        box_width / rKernel.get_key("ros_distance_calculation_constant").value)
                                ...
                                processing_frame = rKernel.cv.putText(processing_frame, class_distance_output,
                                                                      ...
                                                                      rKernel.get_key(
                                                                          ...
                                                                          blue, green, red),
                                                                      rKernel.get_key(
                                                                          "ros_opencv_label_font_thickness").value)

    if rKernel.get_key("ros_fps_indicate_enabled").value:
        fps = round(rKernel.get_fps(), 1)
        if fps > rKernel.get_key("ros_maximize_perfomance_when_fps_drop").value:
            text_y = rKernel.cv.getTextSize("fps", rKernel.cv.FONT_HERSHEY_SIMPLEX,
                                            rKernel.get_key("ros_opencv_label_font_size").value,
                                            rKernel.get_key("ros_opencv_label_font_thickness").value)[0][1]
            processing_frame = rKernel.cv.putText(processing_frame, str(fps) + "fps",
                                                  (0, text_y),
                                                  rKernel.cv.FONT_HERSHEY_SIMPLEX,
                                                  rKernel.get_key("ros_opencv_label_font_size").value, (
                                                      0, 0, 255),
